<?php
if ( ! class_exists( 'CI_Widget_Testimonial' ) ):
	class CI_Widget_Testimonial extends WP_Widget {
		...
		function __construct(){
			$widget_ops  = array( 'description' => esc_html__( 'Displays a testimonial.', 'zermatt' ) );
			$control_ops = array( /*'width' => 300, 'height' => 400*/ );
			parent::__construct( 'ci-testimonial', $name = esc_html__( 'Theme - Testimonial', 'zermatt' ), $widget_ops, $control_ops );
		...
		function widget( $args, $instance ) {
			...
			$random = $instance['random'] == 1 ? true : false;
		...
		function form( $instance ) {
			...
			<p>
				<label><?php _e( 'Testimonial:', 'zermatt' ); ?>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'random' ) ); ?>">
					<input type="checkbox" name="<?php echo esc_attr( $this->get_field_name( 'random' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'random' ) ); ?>" value="1" <?php checked( $random, 1 ); ?> />
					<?php esc_html_e( 'Show random testimonial (ignores the selection above).', 'zermatt' ); ?>
				</label>
			</p>
	...
	register_widget( 'CI_Widget_Testimonial' );
