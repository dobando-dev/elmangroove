<?php
if ( ! class_exists( 'CI_Widget_Page_Gallery' ) ):
	class CI_Widget_Page_Gallery extends WP_Widget {
		...
		function __construct(){
			$widget_ops  = array( 'description' => esc_html__( 'Displays a page that has the "Page with gallery" template assigned.', 'zermatt' ) );
			$control_ops = array( /*'width' => 300, 'height' => 400*/ );
			parent::__construct( 'ci-page-gallery', $name = esc_html__( 'Theme - Page Gallery', 'zermatt' ), $widget_ops, $control_ops );
				...
				while ( $q->have_posts() ) {
					...
					<div class="row">
						<div class="col-sm-6">
							<?php
								$i = 0;
								$gallery = zermatt_featgal_get_attachments();
							?>
							<?php if ( $gallery->have_posts() ) : ?>
								<div class="justified-gallery ci-lightbox-gallery" data-height="160">
									<?php foreach( $gallery->posts as $image ): ?>
										<?php $details = wp_prepare_attachment_for_js( $image->ID ); ?>
										<div>
											<a href="<?php echo esc_url( zermatt_get_image_src( $image->ID, 'large' ) ); ?>" title="<?php echo esc_attr( $details['caption'] ); ?>">
												<img
													src="<?php echo esc_url( zermatt_get_image_src( $image->ID, 'zermatt_height_limit' ) ); ?>"
						...
						<div class="col-lg-5 col-sm-6">
							<div class="entry-content text-lg text-emphasis">
								...
								?>
								<a href="<?php the_permalink(); ?>" class="btn btn-transparent">
									<?php echo esc_html_x( 'Learn More', 'page button', 'zermatt' ); ?>
								</a>
							</div>
						</div>
					</div>
		...
		function update( $new_instance, $old_instance ) {
			...
			$instance['max_img'] = intval( $new_instance['max_img'] ) >= 1 ? intval( $new_instance['max_img'] ) : 3;
		...
		function form( $instance ) {
			...
			<p>
				<label><?php _e( 'Page with gallery:', 'zermatt' ); ?>
					<?php
						zermatt_dropdown_posts( array(
							'post_type'            => 'page',
			...
			<p><?php _e( 'The widget will display the <strong>excerpt</strong> of the page selected, and if empty, it will display the <strong>content</strong> instead.', 'zermatt' ); ?></p>

			<p><label><?php _e( 'Max number of images:', 'zermatt' ); ?></label><input id="<?php echo $this->get_field_id( 'max_img' ); ?>" name="<?php echo $this->get_field_name( 'max_img' ); ?>" type="number" min="1" step="1" value="<?php echo esc_attr( $max_img ); ?>" class="widefat" /></p>
	...
	register_widget( 'CI_Widget_Page_Gallery' );
