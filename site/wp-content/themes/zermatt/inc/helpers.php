<?php
function zermatt_admin_notice_sample_content() {
	...
	$sample_content_url = apply_filters( 'zermatt_sample_content_url',
	...
	?>
	<div class="notice notice-info is-dismissible zermatt-sample-content-notice">
		<p>
			<?php echo wp_kses( sprintf( __( 'It looks like the theme was just installed. <a href="%s" target="_blank">Download the sample content</a> to get things moving.', 'zermatt' ),
	...
	wp_enqueue_script( 'zermatt-sample-content', get_template_directory_uri() . '/inc/js/sample-content.js', array(), $theme->get( 'Version' ), true );

	$settings = array(
		'ajaxurl'       => admin_url( 'admin-ajax.php' ),
		'dismiss_nonce' => wp_create_nonce( 'zermatt-dismiss-sample-content' ),
	);
	wp_localize_script( 'zermatt-sample-content', 'zermatt_SampleContent', $settings );
}

function zermatt_ajax_dismiss_sample_content() {
	check_ajax_referer( 'zermatt-dismiss-sample-content', 'nonce' );
...
function zermatt_wrap_archive_widget_post_counts_in_span( $output ) {
	$output = preg_replace_callback( '#(<li>.*?<a.*?>.*?</a>.*&nbsp;)(\(.*?\))(.*?</li>)#', 'zermatt_replace_archive_widget_post_counts_in_span', $output );

	return $output;
}

function zermatt_replace_archive_widget_post_counts_in_span( $matches ) {
...
function zermatt_wrap_category_widget_post_counts_in_span( $output, $args ) {
	if ( ! isset( $args['show_count'] ) || $args['show_count'] == 0 ) {
		return $output;
	}
	$output = preg_replace_callback( '#(<a.*?>\s*)(\(.*?\))#', 'zermatt_replace_category_widget_post_counts_in_span', $output );

	return $output;
}

function zermatt_replace_category_widget_post_counts_in_span( $matches ) {
 ...
 */
function zermatt_get_page_var( $default_return = 0 ) {
 ...
 */
function zermatt_get_image_src( $image_id, $size = 'full' ) {
 ...
 * @return string
 */
function zermatt_substr_left( $string, $length ) {
 ...
 * @return string
 */
function zermatt_substr_right( $string, $length ) {
	return substr( $string, - $length, $length );
}

if ( ! function_exists( 'zermatt_get_related_posts' ) ):
/**
 ...
 */
function zermatt_get_related_posts( $post_id, $related_count, $args = array() ) {
...
$zermatt_glob_inline_js = array();
if ( ! function_exists( 'zermatt_add_inline_js' ) ):
/**
 ...
 */
function zermatt_add_inline_js( $script, $handle = false ) {
	global $zermatt_glob_inline_js;

	$handle = sanitize_key( $handle );

	if ( ( $handle !== false ) and ( $handle != '' ) ) {
		$zermatt_glob_inline_js[ $handle ] = "\n" . $script . "\n";
	} else {
		$zermatt_glob_inline_js[] = "\n" . $script . "\n";
	}
}
endif;

if ( ! function_exists( 'zermatt_get_inline_js' ) ):
/**
 * Retrieves the inline JS scripts that are registered for printing.
 *
 * @return array The inline JS scripts queued for printing.
 */
function zermatt_get_inline_js() {
	global $zermatt_glob_inline_js;

	return $zermatt_glob_inline_js;
}
endif;

if ( ! function_exists( 'zermatt_print_inline_js' ) ):
/**
 * Prints the inline JS scripts that are registered for printing, and removes them from the queue.
 */
function zermatt_print_inline_js() {
	global $zermatt_glob_inline_js;

	if ( empty( $zermatt_glob_inline_js ) ) {
		return;
	}

	$sanitized = array();

	foreach ( $zermatt_glob_inline_js as $handle => $script ) {
...
if ( ! function_exists( 'zermatt_inflect' ) ):
	/**
	 ...
	 * @return string
	 */
	function zermatt_inflect( $num, $none, $one, $many ) {
		if ( $num == 0 ) {
			return $none;
		} elseif ( $num == 1 ) {
			return $one;
...
if ( ! function_exists( 'zermatt_e_inflect' ) ):
	/**
	 ...
	 * @return void
	 */
	function zermatt_e_inflect( $num, $none, $one, $many ) {
		echo zermatt_inflect( $num, $none, $one, $many );
	}
endif;

if ( ! function_exists( 'zermatt_merge_wp_queries' ) ):
/**
 ...
 */
function zermatt_merge_wp_queries() {
	$args = func_get_args();
	...
	$params = apply_filters( 'zermatt_merge_wp_queries', $params, $args );
...
if ( ! function_exists( 'zermatt_dropdown_posts' ) ):
/**
 ...
 */
function zermatt_dropdown_posts( $args = '', $name = 'post_id' ) {
	$defaults = array(
		'depth'                 => 0,
		'post_parent'           => 0,
		'selected'              => 0,
	...
	$output = apply_filters( 'zermatt_dropdown_posts', $output, $name, $r );
 ...
 */
function zermatt_can_use_plugin( $plugin ) {
